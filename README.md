...
TerSterm 是一个基于 Tauri 2、Rust、Vue 3、Ant Design Vue 和 xterm.js 的桌面 SSH 管理工具，面向需要同时管理多台 Linux 服务器的日常运维场景。

当前仓库版本已经具备连接管理、多终端分屏、同步输入、远程文件管理、Zmodem 传输、深链快速会话、托盘行为控制和 GitHub Release 自更新等能力。
...
- Rust stable
- Tauri 2 构建环境
- 系统中可用的 `ssh` 与 `ssh-keygen`

Windows 下建议直接使用系统自带的 OpenSSH Client，并确保 `ssh.exe`、`ssh-keygen.exe` 可在 `PATH` 中找到。
...
- 前端：Vue 3 + TypeScript + Ant Design Vue + xterm.js
- 桌面容器：Tauri 2
- 后端：Rust
